...
class Crawler:
    ...
    def crawl_snippet_bing_new(self, concept):
        res = []
        ...
        if block is not None:
            ...
            if t and s:
                ...
                # res.append('标题:{} 内容:{}'.format(title, snippet))
                body = {
                    "title": title,
                    "raw_content": snippet,
                }
        ...
        for block in soup.find_all("li", class_="b_algo"):
            ...
            if title and snippet:
                # res.append('标题:{} 内容:{} url:{}'.format(title, snippet,url))
                body = {"title": title, "content": snippet, "url": url}
                res.append(body)
    ...
    def crawl_snippet_new(self, concept):
        self.tot_crawl += 1
        if self.tot_crawl % 100 == 0:
            print("sleep 60s~90s after crawl 100 times")
            sleep(60)
        ...
        if self.config.snippet_source == "bing":
            res = self.crawl_snippet_bing(concept)
        if self.config.snippet_source == "bing_new":
            res = self.crawl_snippet_bing_new(concept)
        return res
    ...
    def get_snippet_rightnow(self, concept):
        res = None
        if not res:
            snippet = self.crawl_snippet_new(concept)
            print(
                "get snippet {} from source {}".format(
                    concept, self.config.snippet_source
                )
